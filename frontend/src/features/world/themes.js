/* Палітри тем світу. Кожен ключ відповідає значенню поля theme у World.
   accentRed / accentGreen — дві «слота» акцентів, якими розфарбовуються
   картки-секції дошки; cover — акцент обкладинки; решта — змінні сторінки. */
export const WORLD_THEMES = {
  sulfur_caves: {
    id: 'sulfur_caves',
    name: 'Сіркові печери',
    pageBg: '#F4FBC3',
    accentRed: '#A63C39',
    accentGreen: '#247A57',
    cover: '#6b7280',
    ink: '#1d1a22',
    soft: 'rgba(0, 0, 0, 0.06)',
    softHover: 'rgba(0, 0, 0, 0.1)',
    activeBg: '#1d1a22',
    activeBgHover: '#333333',
    activeInk: '#ffffff',
    rowBg: 'rgba(0, 0, 0, 0.04)',
    rowLabel: 'rgba(0, 0, 0, 0.5)',
    outline: 'rgba(0, 0, 0, 0.15)',
    outlineHover: 'rgba(0, 0, 0, 0.35)',
    dragOver: '#1d1a22',
    resize: 'rgba(0, 0, 0, 0.25)',
    resizeActive: 'rgba(0, 0, 0, 0.4)',
    dialogBg: '#faf6e0',
    dialogInk: '#1d1a22',
    dialogMuted: '#5f5847',
    dialogOutline: '#b8ad8a',
  },
  amethyst: {
    id: 'amethyst',
    name: 'Аметистовий кристал',
    pageBg: '#dfcefd',
    accentRed: '#9166c8',
    accentGreen: '#ab8cdd',
    cover: '#7a52b0',
    ink: '#1d1a22',
    soft: 'rgba(0, 0, 0, 0.06)',
    softHover: 'rgba(0, 0, 0, 0.1)',
    activeBg: '#1d1a22',
    activeBgHover: '#333333',
    activeInk: '#ffffff',
    rowBg: 'rgba(0, 0, 0, 0.04)',
    rowLabel: 'rgba(0, 0, 0, 0.5)',
    outline: 'rgba(0, 0, 0, 0.15)',
    outlineHover: 'rgba(0, 0, 0, 0.35)',
    dragOver: '#1d1a22',
    resize: 'rgba(0, 0, 0, 0.25)',
    resizeActive: 'rgba(0, 0, 0, 0.4)',
    dialogBg: '#f4edfe',
    dialogInk: '#1d1a22',
    dialogMuted: '#4b4358',
    dialogOutline: '#cdc0dd',
  },
  trial_palace: {
    id: 'trial_palace',
    name: 'Палац випробувань',
    pageBg: '#434343',
    accentRed: '#d57c6a',
    accentGreen: '#54ac98',
    cover: '#7d7188',
    ink: '#ffffff',
    soft: 'rgba(255, 255, 255, 0.12)',
    softHover: 'rgba(255, 255, 255, 0.2)',
    activeBg: '#ffffff',
    activeBgHover: '#e6e6e6',
    activeInk: '#1d1a22',
    rowBg: 'rgba(255, 255, 255, 0.08)',
    rowLabel: 'rgba(255, 255, 255, 0.6)',
    outline: 'rgba(255, 255, 255, 0.3)',
    outlineHover: 'rgba(255, 255, 255, 0.55)',
    dragOver: '#d57c6a',
    resize: 'rgba(255, 255, 255, 0.35)',
    resizeActive: 'rgba(255, 255, 255, 0.6)',
    dialogBg: '#434343',
    dialogInk: '#ffffff',
    dialogMuted: 'rgba(255, 255, 255, 0.72)',
    dialogOutline: 'rgba(255, 255, 255, 0.35)',
  },
  dappled_forest: {
    id: 'dappled_forest',
    name: 'Строкатий ліс',
    pageBg: '#FAE9C6',
    accentRed: '#E8734A',
    accentGreen: '#6B4A3A',
    cover: '#6B4A3A',
    ink: '#1d1a22',
    soft: 'rgba(0, 0, 0, 0.06)',
    softHover: 'rgba(0, 0, 0, 0.1)',
    activeBg: '#1d1a22',
    activeBgHover: '#333333',
    activeInk: '#ffffff',
    rowBg: 'rgba(0, 0, 0, 0.04)',
    rowLabel: 'rgba(0, 0, 0, 0.5)',
    outline: 'rgba(0, 0, 0, 0.15)',
    outlineHover: 'rgba(0, 0, 0, 0.35)',
    dragOver: '#1d1a22',
    resize: 'rgba(0, 0, 0, 0.25)',
    resizeActive: 'rgba(0, 0, 0, 0.4)',
    dialogBg: '#FBF3E0',
    dialogInk: '#1d1a22',
    dialogMuted: '#6b5b3e',
    dialogOutline: '#d8c49a',
  },
  cherry_grove: {
    id: 'cherry_grove',
    name: 'Вишнева роща',
    pageBg: '#F9DEE5',
    accentRed: '#C96F7F',
    accentGreen: '#5A4A52',
    cover: '#8A5A66',
    ink: '#1d1a22',
    soft: 'rgba(0, 0, 0, 0.06)',
    softHover: 'rgba(0, 0, 0, 0.1)',
    activeBg: '#5A4A52',
    activeBgHover: '#4a3d44',
    activeInk: '#ffffff',
    rowBg: 'rgba(0, 0, 0, 0.04)',
    rowLabel: 'rgba(0, 0, 0, 0.5)',
    outline: 'rgba(0, 0, 0, 0.15)',
    outlineHover: 'rgba(0, 0, 0, 0.35)',
    dragOver: '#C96F7F',
    resize: 'rgba(0, 0, 0, 0.25)',
    resizeActive: 'rgba(0, 0, 0, 0.4)',
    dialogBg: '#FBEDF1',
    dialogInk: '#1d1a22',
    dialogMuted: '#7a5f68',
    dialogOutline: '#e3b9c4',
  },
  skies: {
    id: 'skies',
    name: 'Небеса',
    pageBg: '#DCEBF7',
    accentRed: '#6A9CC4',
    accentGreen: '#526078',
    cover: '#5B7A99',
    ink: '#1d1a22',
    soft: 'rgba(0, 0, 0, 0.06)',
    softHover: 'rgba(0, 0, 0, 0.1)',
    activeBg: '#526078',
    activeBgHover: '#434f63',
    activeInk: '#ffffff',
    rowBg: 'rgba(0, 0, 0, 0.04)',
    rowLabel: 'rgba(0, 0, 0, 0.5)',
    outline: 'rgba(0, 0, 0, 0.15)',
    outlineHover: 'rgba(0, 0, 0, 0.35)',
    dragOver: '#6A9CC4',
    resize: 'rgba(0, 0, 0, 0.25)',
    resizeActive: 'rgba(0, 0, 0, 0.4)',
    dialogBg: '#EAF3FA',
    dialogInk: '#1d1a22',
    dialogMuted: '#5a6b7d',
    dialogOutline: '#b9d0e2',
  },
  desert: {
    id: 'desert',
    name: 'Пустеля',
    pageBg: '#F3E3B3',
    accentRed: '#CDAE7C',
    accentGreen: '#8E5F7E',
    cover: '#8A6D3B',
    ink: '#1d1a22',
    soft: 'rgba(0, 0, 0, 0.06)',
    softHover: 'rgba(0, 0, 0, 0.1)',
    activeBg: '#8E5F7E',
    activeBgHover: '#77516a',
    activeInk: '#ffffff',
    rowBg: 'rgba(0, 0, 0, 0.04)',
    rowLabel: 'rgba(0, 0, 0, 0.5)',
    outline: 'rgba(0, 0, 0, 0.15)',
    outlineHover: 'rgba(0, 0, 0, 0.35)',
    dragOver: '#CDAE7C',
    resize: 'rgba(0, 0, 0, 0.25)',
    resizeActive: 'rgba(0, 0, 0, 0.4)',
    dialogBg: '#FAF0D7',
    dialogInk: '#1d1a22',
    dialogMuted: '#6b5f3e',
    dialogOutline: '#d8c49a',
  },
  warm_ocean: {
    id: 'warm_ocean',
    name: 'Океан',
    pageBg: '#D2E9F2',
    accentRed: '#2F5D8A',
    accentGreen: '#3E9C8B',
    cover: '#2E6E81',
    ink: '#123A44',
    soft: 'rgba(18, 58, 68, 0.07)',
    softHover: 'rgba(18, 58, 68, 0.12)',
    activeBg: '#123A44',
    activeBgHover: '#1D4F5C',
    activeInk: '#ffffff',
    rowBg: 'rgba(18, 58, 68, 0.05)',
    rowLabel: 'rgba(18, 58, 68, 0.55)',
    outline: 'rgba(18, 58, 68, 0.18)',
    outlineHover: 'rgba(18, 58, 68, 0.4)',
    dragOver: '#2F5D8A',
    resize: 'rgba(18, 58, 68, 0.3)',
    resizeActive: 'rgba(18, 58, 68, 0.5)',
    dialogBg: '#E9F4F8',
    dialogInk: '#123A44',
    dialogMuted: '#4A6F79',
    dialogOutline: '#B3D2DC',
  },
  pale_garden: {
    id: 'pale_garden',
    name: 'Блідий сад',
    pageBg: '#DBDED3',
    accentRed: '#4E4A44',
    accentGreen: '#9AA198',
    cover: '#6B655E',
    ink: '#2E2B27',
    soft: 'rgba(0, 0, 0, 0.06)',
    softHover: 'rgba(0, 0, 0, 0.1)',
    activeBg: '#2E2B27',
    activeBgHover: '#4A4540',
    activeInk: '#ffffff',
    rowBg: 'rgba(0, 0, 0, 0.04)',
    rowLabel: 'rgba(0, 0, 0, 0.5)',
    outline: 'rgba(0, 0, 0, 0.15)',
    outlineHover: 'rgba(0, 0, 0, 0.35)',
    dragOver: '#4E4A44',
    resize: 'rgba(0, 0, 0, 0.25)',
    resizeActive: 'rgba(0, 0, 0, 0.4)',
    dialogBg: '#E7E9E0',
    dialogInk: '#2E2B27',
    dialogMuted: '#6B665E',
    dialogOutline: '#B9B4A8',
  },
}

export const DEFAULT_THEME_ID = 'sulfur_caves'

export function getWorldTheme(themeId) {
  return WORLD_THEMES[themeId] || WORLD_THEMES[DEFAULT_THEME_ID]
}

/* CSS-змінні для діалогів (редагування світу, тема), щоб вони
   підлаштовувались під палітру поточної теми світу. */
export function themeDialogStyle(themeId) {
  const t = getWorldTheme(themeId)
  return {
    '--accent': t.accentRed,
    '--dialog-bg': t.dialogBg,
    '--dialog-ink': t.dialogInk,
    '--dialog-muted': t.dialogMuted,
    '--dialog-outline': t.dialogOutline,
  }
}
